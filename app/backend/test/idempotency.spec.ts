...
import { describe, it, expect, beforeAll, afterAll } from '@jest/globals'; // Explicit Jest imports
...
beforeAll(async () => {
  ...
  app.post('/v1/transactions/submit', idempotencyMiddleware(store), submitTransaction);
});
...
it('Missing key returns 400', async () => {
  const res = await request(app).post('/v1/transactions/submit').send(validBody);
  expect(res.status).toBe(400);
  expect(res.body.error).toContain('Missing');
});

it('Invalid key returns 400', async () => {
  const res = await request(app).post('/v1/transactions/submit').set('Idempotency-Key', 'bad key!').send(validBody);
  expect(res.status).toBe(400);
});

it('First request succeeds', async () => {
  const res = await request(app).post('/v1/transactions/submit').set('Idempotency-Key', 'key-1').send(validBody);
  expect(res.status).toBe(200);
  expect(res.headers['x-idempotent-replayed']).toBeUndefined();
});

it('Duplicate request replays cached response', async () => {
  const res1 = await request(app).post('/v1/transactions/submit').set('Idempotency-Key', 'key-2').send(validBody);
  const res2 = await request(app).post('/v1/transactions/submit').set('Idempotency-Key', 'key-2').send(validBody);
...
it('Mismatched body returns 409', async () => {
  await request(app).post('/v1/transactions/submit').set('Idempotency-Key', 'key-3').send(validBody);
  const res = await request(app).post('/v1/transactions/submit').set('Idempotency-Key', 'key-3').send({ transactionXdr: 'B' });
...
it('Processing record returns 409', async () => {
  ...
  expect(res.status).toBe(409);
  expect(res.body.error).toContain('processed'); // <--- Changed from 'processing' to 'processed'
});


it('GET /v1/transactions/:hash returns 404', async () => {
  // The test app only has the POST route, so we need to add the GET route temporarily
  // or just test it if it's already in your app setup.
  // Assuming you added it to your app setup in beforeAll:
...
it('Handles request body with arrays for fingerprinting', async () => {
  const bodyWithArray = { transactionXdr: 'AAAA', args: [1, 2, 3] };
  const res = await request(app)
    .post('/v1/transactions/submit')
...
it('Too long key returns 400', async () => {
  const longKey = 'a'.repeat(129); // Max is 128
  const res = await request(app)
    .post('/v1/transactions/submit')
...
it('Cleanup deletes expired records', async () => {
  await pool.query(`
    ...
    VALUES ($1, $2, 'succeeded', now() - interval '48 hours', now() - interval '48 hours')
